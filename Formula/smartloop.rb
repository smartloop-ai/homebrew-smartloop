class Smartloop < Formula
  desc "Local AI orchestration to extract information and generate content"
  homepage "https://smartloop.ai"
  url "https://github.com/smartloop-ai/smartloop-tui/archive/refs/tags/v1.0.1.tar.gz"
  sha256 "925221bebc041fc02d6dccdc13c754d2758958cd1083ab3548befe9fa0fe0204"
  license "Apache-2.0"
  ...
  def install
    ...
    cp "version.py", venv_site/"version.py"
    cp "main.py", venv_site/"main.py"

    (bin/"slp").write <<~EOS
      #!/bin/bash
      export SPATIALINDEX_C_LIBRARY="#{Formula["spatialindex"].opt_lib}/libspatialindex_c.#{OS.mac? ? "dylib" : "so"}"
      export API_PORT=0
